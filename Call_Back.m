% Callback functions
function [PGA,PGV,Ia,D_5_75,D_5_95]=Call_Back(Mw, VS30, RJB, FM, FD, ax, Param_Max, Param_Min, Net_Save, fig)
format long g

    % predict net
    Input_case_study=[FD FM Mw RJB VS30];
    % Inputs are normalized using the following equation: 0.60*(xi-xmin)/(xmax-xmin)+0.20
    for i_row=1:1 %No_Par
        Inputs_case_study_NML(i_row,:)= 0.60*( (Input_case_study(i_row,:)-Param_Min(i_row,:)) ...
            / (Param_Max(i_row,:)-Param_Min(i_row,:)) ) + 0.2; %#ok<AGROW,SAGROW>
    end
    ...
    PGV       = round(exp(Outputs_Mean_case_study(2))*1,2);
...
% uilabel(fig, 'Text', '$PSa\;_{0.2\;s}$','Interpreter','latex', 'Position', [215 490 80 20]);
uieditfield(fig, 'numeric', 'Position', [325 490 50 20],'Value',PSa_02sec);

% uilabel(fig, 'Text', '$PSa\;_{0.5\;s}$','Interpreter','latex', 'Position', [215 465 80 20]);
uieditfield(fig, 'numeric', 'Position', [325 465 50 20], 'Value', PSa_05sec);

% uilabel(fig, 'Text', '$PSa\;_{1.0\;s}$','Interpreter','latex', 'Position', [215 440 80 20]);
uieditfield(fig, 'numeric', 'Position', [325 440 50 20], 'Value', PSa_10sec);

%  uilabel(fig, 'Text', '$PSa\;_{2.0\;s}$','Interpreter','latex', 'Position', [215 415 80 20]);
 uieditfield(fig, 'numeric', 'Position', [325 415 50 20], 'Value', PSa_20sec);

%  uilabel(fig, 'Text', '$PSa\;_{4.0\;s}$','Interpreter','latex', 'Position', [350 490 80 20]);
 uieditfield(fig, 'numeric', 'Position', [500 490 50 20], 'Value', PSa_40sec);

%    uilabel(fig, 'Text', '$PGA$','Interpreter','latex', 'Position', [350 465 80 20]);
 uieditfield(fig, 'numeric', 'Position', [500 465 50 20], 'Value', PGA);

%    uilabel(fig, 'Text', '$PGV$','Interpreter','latex', 'Position', [350 440 80 20]);
 uieditfield(fig, 'numeric', 'Position', [500 440 50 20], 'Value', PGV);

%    uilabel(fig, 'Text', '$CAV$','Interpreter','latex', 'Position', [350 415 80 20]);
 uieditfield(fig, 'numeric', 'Position', [500 415 50 20], 'Value', CAV);

%   uilabel(fig, 'Text', '$I_a$','Interpreter','latex', 'Position', [475 490 80 20]);
 uieditfield(fig, 'numeric', 'Position', [675 490 50 20], 'Value', Ia);

%    uilabel(fig, 'Text', '$T_m$','Interpreter','latex', 'Position', [475 465 80 20]);
 uieditfield(fig, 'numeric', 'Position', [675 465 50 20], 'Value', T_m);

%     uilabel(fig, 'Text', '$D_{5\%-75\%}$','Interpreter','latex', 'Position', [475 440 80 20]);
 uieditfield(fig, 'numeric', 'Position', [675 440 50 20], 'Value', D_5_75);

%     uilabel(fig, 'Text', '$D_{5\%-95\%}$','Interpreter','latex', 'Position', [475 415 80 20]);
